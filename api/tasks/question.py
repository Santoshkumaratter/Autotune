...
def generate_questions(redis, task_id, req: QuestionCreationRequest, openai_key):
    data = {"data": []}
    try:
...
def generate_and_push_questions(
    redis, task_id, req: QuestionCreationRequest, openai_key, huggingface_key
):
    data = generate_questions(redis, task_id, req, openai_key)
    redis.hset(
        task_id,
        mapping={
            "status": "Completed",
    ...
    push_questionset_to_hf(redis, task_id, req, huggingface_key, data)


def push_questionset_to_hf(redis, task_id, req, huggingface_key, data):
    train, test, val = {}, {}, {}
...
def generate_and_update_questions(
    redis, task_id, req: QuestionUpdationRequest, openai_key, huggingface_key
):
    data = generate_questions(redis, task_id, req, openai_key)

    if req.bulk_process:  # for multiple put requests, update HF once every 50 requests
        if req.index % 50 == 0:
            upload.get_redis_keys_status(redis, req, huggingface_key, task_id)
    else:
