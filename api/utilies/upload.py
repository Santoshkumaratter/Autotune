...
import pandas as pd
from huggingface_hub import HfApi, HfFolder, HfRepository
from rest_framework.exceptions import APIException

logger = logging.getLogger(__name__)
logger.setLevel(logging.INFO)

class SplitDataException(APIException):
    status_code = 400
    default_detail = "Error occurred while splitting data."
        ...
        try:
            ...
            if key_type == "hash":
                ...
                if hash_values["status"] == "Generated":
                    ...
                    logger.info("Prepped for upload")
...
async def push_dataset_to_hf(
    ...
    try:
        hf_api = HfApi()
        repo_name = f"{req.repo}-generated-dataset"
        repo = HfRepository(repo_name)
        for split, df in zip(["train", "validation", "test"], [train, val, test]):
            path = f"datasets/{req.repo}/{split}.csv"

            original_data = repo.download(path, revision=req.version)
            original_data = pd.read_csv(StringIO(original_data.decode("utf-8")))
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        await get_redis_keys_status(redis_pool)
