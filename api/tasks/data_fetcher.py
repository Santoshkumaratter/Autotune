...
class DataFetcher:
    ...
    def __init__(self, req, openai_key, redis, task_id, questions=False):
        self.req = req
        self.openai_key = openai_key
        self.redis = redis
        self.task_id = task_id
        self.data = {"data": []}
        self.semaphore = threading.Semaphore(self.MAX_CONCURRENT_FETCHES)
        self.status = None
        self.iteration = 0
        self.questions = questions

    def _initialize_from_redis(self):
        existing_data = self.redis.hgetall(self.task_id)
    ...
    def _fetch_data(self):
        ...
        logger.info("Iteration Completed. Current Total samples %d", len(self.data["data"]))

        if len(self.data["data"]) < num_samples:
            logger.info("Need to fetch more data. Starting new iteration for task %s", self.task_id)
            self.iteration += 1
            self._fetch_data()
